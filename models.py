...
class GAT(nn.Module):
    def __init__(self, in_feats, h_feats, num_classes):
        ...
        self.conv2 = GATConv(h_feats, num_classes, num_heads=1)

    def forward(self, g, in_feat):
        h = self.conv1(g, in_feat)# torch.Size([11944, 1, 16])
        h = h.squeeze(1)# torch.Size([11944, 16])
        h = F.relu(h)
        h = self.conv2(g, h) # torch.Size([11944, 1, 1])
        h = h.squeeze(1) # torch.Size([11944, 1])
        h = in_feat * F.softplus(h) # torch.Size([11944, 2])
        return h
